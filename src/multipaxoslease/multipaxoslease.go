...
func (r *Replica) handlePropose(propose *genericsmr.Propose) {
	// Got a read command. We assume the leader has the lease, so the leader can directly return to the client
	if propose.Command.Op == state.GET {
		...
		// Needed for do local test on a single server
		//time.Sleep(1)
